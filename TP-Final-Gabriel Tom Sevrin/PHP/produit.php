...
// NOTE IMPORTANTE: J'ai SIMULÉ la colonne quantite (stock) avec un nombre aléatoire (rand(0, 50)) 
// pour que le code d'affichage du stock (Problème 6) fonctionne. 
// Pour un fonctionnement réel, vous devez ajouter une colonne 'quantite' (INT) à votre table 'produit'.
...
if ($result && $result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        // Simuler le stock pour le point 4
        $stock = rand(0, 50);
        ...
        $products_from_db[] = [
            ...
            'price'       => $row['prix'],
            'image'       => $row['imageNom'],
            // SIMULÉ: Le stock réel doit venir d'une colonne de la BDD
            'quantite'    => $stock,
        ];
    }
}
...
// -----------------------------------------------------------------------------
// ⚙️ EXPORTER LES DONNÉES EN JSON POUR LE JAVASCRIPT (FIX 3: AUTOCOMPLETION)
// -----------------------------------------------------------------------------
            ...
            theme: {
                extend: {
                    colors: {
                        ...
                        'card-bg': '#1e1e1e',
                        'error-red': '#ff4444',
                        /* AJOUTÉ POUR LE STOCK FAIBLE */
                        'success-green': '#00ff7f',
                        /* AJOUTÉ POUR LE BON STOCK */
                    },
                    fontFamily: {
...
<body class="bg-dark-bg">
    ...
    <section class="main-content min-h-screen pt-24 pb-8 bg-dark-bg">
        <main class="container mx-auto px-4 max-w-7xl">
            ...
            <p class="text-center text-gray-400 mb-2 font-open-sans max-w-2xl mx-auto">
                        ...
                        <div class="product-card bg-card-bg border-2 border-neon-cyan/50 rounded-2xl p-6 shadow-xl 
                            ...
                            <div class="flex flex-col gap-3">
                                ...
                                <button class="add-to-cart-btn w-full py-3 rounded-full bg-neon-cyan text-dark-bg font-orbitron uppercase tracking-wider transition-all duration-300 hover:bg-white hover:shadow-lg shadow-neon-btn"
                                    ...
                                    data-product-image="../IMG/<?= htmlspecialchars($product['image']) ?>"
