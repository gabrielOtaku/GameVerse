<?php
// TP-Final-Gabriel Tom Sevrin/include/header.inc.php
...
if ($is_logged_in) {
    ...
    if ($photo_data_b64 && $photo_type) {
        ...
        // Chemin relatif corrigé pour être inclus dans un fichier PHP
...
if ($is_logged_in) {
    // AFFICHAGE DU PROFIL CONNECTÉ (Bouton Déconnexion)
    ...
    if ($photo_src) {
        ...
        echo '<i class="fas fa-user-circle profile-avatar"></i>'; // Icône par défaut si pas de photo
    }
    ...
    echo '<a class="nav-link-transition" href="profil.php">Mon Profil</a>';
    // Chemin corrigé pour la déconnexion
    echo '<a class="nav-link-transition" href="Deconnexion.php">Déconnexion</a>';
    echo '</div>';
} else {
...
if ($is_logged_in && isset($_SESSION['new_login']) && $_SESSION['new_login'] === true) {
    ...
    echo '</div>';
    echo '</div>';
    // Supprimer le flag après l'affichage pour qu'il n'apparaisse pas au prochain rechargement
    unset($_SESSION['new_login']);
}


// --- POP-UP DE PARAMÈTRES (Inchangé, mais s'assurer qu'il est cohérent) ---
