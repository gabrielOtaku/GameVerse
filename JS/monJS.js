...
// -------------------------------------------------------------
// --- FONCTIONS DU PANIER (Overlay et Badge) ---
// -------------------------------------------------------------
const cartBtnOverlay = document.getElementById("cart-btn-overlay");
const cartOverlay = document.getElementById("cart-overlay");
const cartOverlayBg = document.getElementById("cart-overlay-bg");
const closeCartBtn = document.getElementById("close-cart-btn");
const cartContentOverlay = document.getElementById("cart-content-overlay");
...
/**
 * Met à jour l'interface utilisateur du panier (badge et overlay).
 * @param {object} data - Contient count, total, et products.
 */
function updateCartUI(data) {
  ...
  // Mise à jour du contenu de l'overlay (Résumé de commande)
  if (cartContentOverlay && goToCheckoutBtn) {
    if (data.products && data.products.length > 0) {
      let html = '<ul class="space-y-4">';
      data.products.forEach((item) => {
        // Détermine le chemin correct pour l'image
        let imagePath = window.location.pathname.includes("/PHP/")
          ? `../IMG/${item.imageNom}`
          : `IMG/${item.imageNom}`;
        html += `
          <li class="flex items-center space-x-4 p-2 bg-card-bg rounded-lg border border-neon-cyan/20">
            <img src="${imagePath}" alt="${
          item.nomProduit
        }" class="w-12 h-12 object-cover rounded-md border border-neon-cyan/50">
            <div class="flex-grow">
              <h4 class="text-sm font-orbitron text-white">${
                item.nomProduit
              }</h4>
              <p class="text-xs text-neon-cyan">${item.quantite} x $${
          item.prix
        }</p>
            </div>
            <span class="text-white font-bold">$${(
              parseFloat(item.prix) * parseInt(item.quantite)
            ).toFixed(2)}</span>
          </li>
        `;
      });
      html += "</ul>";
      cartContentOverlay.innerHTML = html;

      // Afficher le bouton de paiement
      goToCheckoutBtn.classList.remove(CLASS_HIDDEN);
    } else {
      cartContentOverlay.innerHTML =
        '<p class="empty-cart-msg text-center text-gray-500 py-10">Votre panier est vide.</p>';
      goToCheckoutBtn.classList.add(CLASS_HIDDEN);
    }
  }

  // Mise à jour du total
  if (cartTotalOverlay) {
    cartTotalOverlay.textContent = `$${data.total.toFixed(2)}`;
  }
}

/**
 * Récupère les données complètes du panier via AJAX et met à jour l'interface.
 */
async function loadCartData() {
  // Détermine le chemin du script AJAX
  const url = window.location.pathname.includes("/PHP/")
    ? "get_cart_data.php"
    : "PHP/get_cart_data.php";
  try {
    ...
    if (data.success) {
      ...
      // Cas où l'utilisateur n'est pas connecté ou autre erreur
      updateCartUI({
        count: data.count || 0,
    ...
    updateCartUI({ count: 0, total: 0.0, products: [] });
  }
}
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // ----------------------------------------------------
  // 2. MENU DEROULANT AUTHENTIFICATION (FIXÉ)
  // ----------------------------------------------------
  ...
  if (authBtn && authMenu && authContainer) {
    // Logique d'affichage du menu par clic
    ...
    document.addEventListener("click", (event) => {
      // Vérifie si le clic est en dehors du conteneur du bouton/menu
      if (!authContainer.contains(event.target)) {
        //si le menu est actuellement ouvert
        if (authMenu.classList.contains(CLASS_SHOW)) {
    ...
    fetch(url, {
      ...
      .then((data) => {
        if (data.success) {
          ...
          showCustomMessage(
            `Erreur: ${
              data.message || "Problème d'ajout au panier. (Connexion requise?)"
            }`,
            productImage,
            0,
            true
      ...
      .catch((error) => {
        ...
        showCustomMessage(
          "Erreur réseau lors de l'ajout au panier.",
          productImage,
          0,
          true
        );
  ...
  // -------------------------------------------------------------
  // ✅ Logique d'ouverture/fermeture de l'Overlay du Panier
  // -------------------------------------------------------------
  if (cartBtnOverlay && cartOverlay && closeCartBtn && cartOverlayBg) {
    const closeOverlay = () => {
      cartOverlay.classList.remove(CLASS_ACTIVE);
      cartOverlayBg.classList.remove(CLASS_ACTIVE);
      setTimeout(() => {
        cartOverlayBg.classList.add(CLASS_HIDDEN);
      }, 400);
    };

    // S'assurer que le bouton ouvre l'overlay
    cartBtnOverlay.addEventListener("click", () => {
      cartOverlay.classList.add(CLASS_ACTIVE);
      cartOverlayBg.classList.remove(CLASS_HIDDEN);
      cartOverlayBg.classList.add(CLASS_ACTIVE);
      loadCartData(); // Recharge les données à l'ouverture
    });
    ...
    // Fermeture par touche Échappe
    document.addEventListener("keydown", (e) => {
      if (
        e.key === "Escape" &&
        cartOverlayBg.classList.contains(CLASS_ACTIVE)
      ) {
        closeOverlay();
      }
    });

    // Si le lien Panier dans la nav existe, il doit aussi ouvrir l'overlay
    const navPanierLink = document.getElementById("nav-panier-link");
    if (navPanierLink) {
      navPanierLink.addEventListener("click", (e) => {
        e.preventDefault(); // Empêche la redirection normale
        cartBtnOverlay.click(); // Déclenche l'ouverture de l'overlay
      });
    }
  }
  ...
  document
    ...
    .forEach((link) => {
      // Exclure le bouton qui ouvre l'overlay (s'il existe)
      if (link.id === "cart-btn-overlay") return;
  ...
  const playBotAnimation = (name, loop = false) => {
    if (botModel) {
      ...
      // Si ce n'est pas un loop, ajoute un listener pour revenir à l'état d'inactivité
      if (!loop) {
  ...
  if (registrationForm) {
    ...
    if (botModel) {
      ...
      // Répéter aléatoirement une animation idle
      setInterval(() => {
        playIdleRandomAnim();
      }, 25000);
    }
    ...
    if (suggestPasswordBtn) {
      suggestPasswordBtn.addEventListener("click", () => {
        ...
        let password = "";
        // Limite la suggestion à 12 caractères
        for (let i = 0; i < 12; i++) {
    ...
    const showRegPopup = (popup, anim) => {
      ...
      playBotAnimation(anim); // Joue l'animation d'erreur ou de succès
    };
    ...
    if (regSuccessCloseBtn) {
      regSuccessCloseBtn.addEventListener("click", () => {
        hideRegPopup(regSuccessPopup, true);
      });
    }
    ...
    loginForm.addEventListener("submit", async (e) => {
      ...
      try {
        ...
        if (result.success) {
          ...
          // Assure la fermeture du popup d'erreur
  ...
  const handleSessionMessages = () => {
    ...
    if (urlParams.has("login") && urlParams.get("login") === "success") {
      ...
      if (successPopup) {
        const successMessage = document.getElementById("success-message");
        if (successMessage) {
          successMessage.innerHTML = "👋 Déconnexion réussie. À bientôt !";
        }
...
function initializeSocialLinks() {
  ...
  if (socialLinksContainer) {
    // Évite d'ajouter les liens plusieurs fois si le script est rechargé
